...
#'                                 The \code{verbose} argument of the [bppg::.minimizeSquaredError()] function.
#' @param control                  \strong{list} \cr
#'                                 The \code{control} object to be passed to the [bppg::.minimizeSquaredError()] function.
...
#'                                 The \code{log_level} argument will passed to the [bppg::.minimizeSquaredError()] function.
#'
#' @return
...
#' @export
#'
#' @seealso [bppg::.minimizeSquaredError()], [bppg::automatedAnalysisIteratedCi()]
#'
#' @details
#' With .minimizeSquaredError() each protein node receives one estimate for the protein ratio. However, in some cases, there are multiple possible values for the protein ratios that lead to the same, minimal error term.
...
#' For each protein node, the Ci value if fixed on a point on the grid, while the other Ci values and all Ri values are optimized using .minimizeSquaredError().
...
#'                                          The list resulting from the [bppg::iterateOverCi()] function.
  ...
  for (i in 1:nr_proteins) {
    ...
    if (abs(diff(range(X_tmp$error))) <= 1e-10) {
      ...
      if (abs(diff(range(R))) > 1e-4) {
        ...
        if (verbose) print(paste0("Constant Solution for R", i, ": ", bppg::.geomMean(R)))
      ...
      if (length(ind_min_tol) <= 1) {
        ...
        if (all(R[ind_min_tol] == 0) | abs(diff(range(log2(R[ind_min_tol])))) < 1e-4) {  ## Ri is constant but Ci is not
          ...
          D_tmp$Ri_max <- NA
          D_tmp$case <- 4
          if (verbose) print(paste0("Constant Solution for R", i, ": ", bppg::.geomMean(R[ind_min_tol])))
