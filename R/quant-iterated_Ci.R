...
# S: single Submatrix
# grid.size: number of values for C, between 0 and 1
# omit_grid_borders: if TRUE, 0 and 1 are excluded from the grid
...
#
# ### Version 2: pro Protein auch die Ergebnisse der anderen Proteine im gleichen System verwenden!
#
#
# load("results/D2_without_isoforms_quant/iterated_Ci/D2_1_9/Solutions_iterated_Ci_v8_extendedgridatborders_12.RData")
# load("data/D2_without_isoforms/D2_quant/preprocessed/Submatrices.RData")
#
# source("R Scripts/iterated_Ci/plot_result_iterated_Ci.R")
# source("R Scripts/protein_quantification/minimize_squared_error_new_v8_log_level.R")
# source("R Scripts/iterated_Ci/iterate_over_different_Cis_v8.R")
#

################################################################################
# v3: erstmal nur die Ergebnisse des aktuellen Peptids anschauen und dann
# später gutcken, ob es sich durch die anderen Peptide erweitern lässt


################################################################################
### TODO: einbauen, dass Ergebnisse vom Protein selbst und von den anderen Proteinen
###       in der Ergebnistabelle getrennt aufgeführt werden!
### TODO: Es wäre zu Überlegen, ob für ein Protein mit uniquen Peptiden
###       immer das geom. Mittel der uniquen Peptide festgelegt werden sollte?
###       Überprüfen, ob das am Ende wirklich immer die optimale Lösung ist??
###       Vielleicht nicht, wenn es sehr viele gesharte Peptide gibt, da es dann
###       nicht so viel bringt, wenn der Fehler des uniquen Peptides = 0 ist
###       (wenn es viele unique Peptide gibt ist minimierung des Fehlers dort sicher hilfreich).



automated_analysis_iterated_Ci <- function(S, res, use_results_from_other_proteins = FALSE,
                                           verbose = FALSE, job = NULL, calc_op_sol = TRUE,
                                           S_is_graph = FALSE) {

  # transform graph into incidence matrix is necessary
  if (S_is_graph & !is.null(S)) {
    X <- igraph::as_incidence_matrix(S)
    fc <- na.omit(igraph::V(S)$pep_ratio)
    S <- list(X = X, fc = fc)
  }
  ...
  if (calc_op_sol) {
    solution_optimal <- minimize_squared_error_v8(S, error.type = "multiplicative", fixed.Ci = NULL,
  ...
  for (i in 1:nr_proteins) {
    ...
    if (abs(diff(range(X_tmp$error))) <= 1e-10) {
      ...
      ### check, das Optimierung ohne Einschränkungen wirklich zum optimalen Ergebnis führt!
      ...
      ### TODO: nicht warnen, wenn Fehler sehr klein (nahe an 0 ist!)  oder Abstand der beiden Fehler sehr sehr gering

      ind_min <- which.min(error)
      ind_min_tol <- which(abs(min(error) - error) <= 1e-10)  ### the error is constant in which area?

      if (length(ind_min_tol) <= 1) {   ## höchstens ein weiterer Punkt, der dem optimalen Fehler nahe kommt

        D_tmp$error_constant <- "no"
        D_tmp$Ri <- R[ind_min_tol]#Ri_optimal[i]
        D_tmp$Ri_min <- NA
        D_tmp$Ri_max <- NA
        D_tmp$Ci <- C[ind_min_tol]#Ci_optimal[i]
        D_tmp$Ci_min <- NA
        D_tmp$Ci_max <- NA
        D_tmp$case <- 3
        ...
        #### TODO: Message anpassenb ohne Ri_optimal und Ci_optimal zu verwenden!
        if (verbose)print(paste0("Single optimal solution is R", i, "= ", Ri_optimal[i], " and C", i, " = ", Ci_optimal[i], "."))
      } else {  ### mehrere Punkte mit quasi konstantem optimalen Fehler
        ### TODO: evtl könnte man hier noch näher an Ci = 1 herangehen und austesten, um die wirkliche obere Grenze zu erzielen.
        ### TODO: das gleiche gilt für die untere Grenze

        ### TODO: D3_1_2_10 protein 2: ind_min_tol ist 276 306 373 442, also nicht nebeneinander liegend!
        ###       Nachvollziehen, was dazwischen passiert, evtl. mit geringerer Toleranz`??`
        ###       -> Man muss aufpassen, dass der Fehler zwischendurch nicht wieder wesentlich ansteigt,
        ###       weil dann hätte man ggfls. kein Intervall mehr für Ri!!

        D_tmp$error_constant <- "partially"

        if (verbose) print(paste0("Multiple points with optimal error."))

        if (abs(diff(range(log2(R[ind_min_tol])))) < 1e-4) {  ## Fall, dass Ri konstant ist, aber Ci nicht!
          D_tmp$Ri <- bppg::geom_mean(R[ind_min_tol])
          D_tmp$Ri_min <- NA
          D_tmp$Ri_max <- NA
          D_tmp$case <- 4
          if (verbose) print(paste0("Constant Solution for R", i, ": ", bppg::geom_mean(R[ind_min_tol])))
        } else {  # Fall dass Ri nicht konstant ist!
          D_tmp$Ri <- NA
        ...
        # print(paste0(range(C[ind_min_tol2])))
      }
    }


    ### TODO: auch Werte für Ci anpassen!
    if (use_results_from_other_proteins) {
      ### gucken, ob sich durch die Daten der anderen Proteine die Lösung erweitern lässt:
      ind <- which(abs(min(error) - X_tmp3$error) <= 1e-10)
      if (length(ind) > 0) {
        if (!is.na(D_tmp$Ri)) {
          if (abs(diff(range(log2(c(D_tmp$Ri, R_3[ind]))))) >= 1e-4) {  # falls es vorher 1 Lösung gab, aber es jetzt mehrere gibt

            D_tmp$Ri_min <- min(c(D_tmp$Ri, R_3[ind]))
            D_tmp$Ri_max <- max(c(D_tmp$Ri, R_3[ind]))
            D_tmp$Ri <- NA
          }
        } else {                                          # falls der Range erweitert werden kann
  ...
  # RES <- cbind(Accession = colnames(S$X), RES)
